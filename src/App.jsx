...
const getSafeLogo = (team, divisions) => {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
        ...
        <text x="50" y="72" font-family="sans-serif" font-size="65" font-weight="900" fill="url(#g)" stroke="${color}" stroke-width="1" text-anchor="middle" filter="url(#ds)">${letter}</text>
    </svg>`;
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-matriz bg-[#020617] text-white">
            ...
            <div className="glass-panel-heavy p-8 rounded-[2.5rem] w-full max-w-md border border-cyan-500/20 animate-scale-in relative overflow-hidden shadow-[0_0_50px_rgba(6,182,212,0.15)]">
                ...
                {/* BOTÓN BYPASS (SOLICITADO) */}
                <div className="mt-6 pt-4 border-t border-cyan-500/10 text-center relative z-10">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#020617] px-2 text-[8px] text-slate-500 uppercase tracking-widest font-jetbrains">Pruebas</div>
                    <button
                        type="button"
                        onClick={onDevBypass}
                        className="w-full bg-[#020617] hover:bg-emerald-900/20 border border-emerald-500/30 hover:border-emerald-500/50 rounded-xl py-3 text-[10px] font-black uppercase text-emerald-400 transition-all flex items-center justify-center gap-2 shadow-inner"
                    >
                        <Key size={14} />
            ...
            <div className={`w-full rounded-[2rem] overflow-hidden relative glass-panel shadow-[0_15px_40px_rgba(0,0,0,0.6)] group transition-all duration-500 ${isOpen ? 'h-40 sm:h-56 md:h-72 border border-cyan-500/20' : 'h-0 border-0 opacity-0'}`}>
                {ads.map((ad, i) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {ad.text && (
                                <div className="absolute bottom-6 left-6 glass-panel-heavy px-5 py-3 rounded-2xl border border-cyan-500/30 shadow-2xl max-w-[85%] backdrop-blur-xl pointer-events-none">
                            ...
                            {ad.url && (
                                <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-cyan-500/30 flex items-center gap-1.5 shadow-lg pointer-events-none text-cyan-400">
                ...
                {ads.length > 1 && (
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                        {ads.map((_, i) => (
                            <div key={i} className={`w-2 h-2 rounded-full transition-all duration-500 ${i === currentIndex ? 'bg-cyan-400 scale-125 shadow-[0_0_10px_#06b6d4]' : 'bg-slate-600'}`} />
                        ))}
                    </div>
                )}
            </div>
                        ...
                        <div className="flex overflow-x-auto snap-x custom-scrollbar px-4 gap-5 pb-2">
                            {userMessages.map(msg => (
                                <div key={msg.id} className="snap-center shrink-0 w-[85vw] max-w-[380px] glass-panel border border-cyan-500/30 rounded-3xl overflow-hidden flex flex-col shadow-2xl relative">
                                    ...
                                    {msg.text && (
                                        <div className="p-5 bg-gradient-to-b from-transparent to-[#020617] relative z-20 -mt-10">
...
export default function App() {
    ...
    const [isDevBypassMode, setIsDevBypassMode] = useState(false); // ESTADO DEL BYPASS
    ...
    // FUNCIÓN DEL BYPASS DE DESARROLLO
    const handleQuickDevEntry = () => {
        ...
        setAppMode('admin');
        setAuthChecked(true);
        setIsDataLoaded(true); // Forzar carga
    ...
    const downloadCanvasHelper = (canvas, filename) => {
        ...
        link.href = canvas.toDataURL('image/png', 1.0); // Exportación HD Pura
        link.click();
    ...
    useEffect(() => { 
        ...
        return () => unsub(); 
    }, [user, showToast, isDevBypassMode]);
    ...
    const updateJornadaMatch = useCallback((tempId, field, value) => {
        setJornadaSummary(p => p.map(m => {
            ...
            // Recalcula los puntos automáticamente si editas los goles
        ...
        const tId = setTimeout(async () => {
            try {
                ...
                if (renderStudio.type === 'viral' || renderStudio.type === 'program') {
                    ...
                    // Fondo adaptado a la altura dinámica (Oscuro estricto)
                    ...
                    if (renderStudio.type === 'viral') {
                        ...
                        const md = renderStudio.data; 
                        const rh = 320, g = 70, sy = 1000;
                        for (let i = 0; i < md.length; i++) { // Bucle completo dinámico
                            const m = md[i];
                            const y = sy + i * (rh + g); 
                ...
                // Actualizar la vista de la UI del modal de render:
                            ...
                            {uiState.view === 'config' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in flex-1">
                                    <div className="glass-panel p-6 rounded-3xl flex flex-col gap-5 shadow-[0_15px_40px_rgba(0,0,0,0.6)] h-fit relative overflow-hidden border border-cyan-500/30">
                                        ...
                                        <div className="flex flex-col gap-2 relative z-10">
                                            <label className="text-[10px] font-black text-cyan-500 uppercase tracking-widest font-jetbrains">Sub-Identificador</label>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-start">
                            ...
                            <div className="flex-1 flex flex-col gap-4 w-full">
                                ...
                                <div className="flex flex-col sm:flex-row gap-3">
                                    ...
                                    <label className="flex-1 bg-[#020617] border border-cyan-500/50 hover:bg-cyan-500/10 py-3 rounded-xl text-[11px] uppercase font-black tracking-widest text-cyan-400 flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 shadow-inner"><Users size={16}/> Recorte Múltiple (Manual)<input type="file" accept="image/*" className="hidden" onChange={e => handleGroupPhotoSelect(e, activeTeamForRoster.id)} /></label>
...
{/* ==================================================== */}
            {/* MODAL 1: EXPEDIENTE DEL JUGADOR (CORRIGE EL CRASH AL ENTRAR AL JUGADOR) */}
            {/* ==================================================== */}
            ...
            {/* ==================================================== */}
            {/* MODAL 2: BUFFER DE JORNADA (CORRIGE EL BOTÓN AL TERMINAR LIVE) */}
            {/* ==================================================== */}
